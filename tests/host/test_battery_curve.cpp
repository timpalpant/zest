...
constexpr std::array too_short{CurvePoint{4200, 100}};
static_assert(validate_curve(too_short).error() == CurveError::insufficient_points);

constexpr std::array ascending{CurvePoint{3700, 100}, CurvePoint{4200, 0}};
static_assert(validate_curve(ascending).error() == CurveError::invalid_voltage_order);

constexpr std::array rising_percent{CurvePoint{4200, 10}, CurvePoint{3700, 90}};
static_assert(validate_curve(rising_percent).error() == CurveError::invalid_percentage);

constexpr std::array over_100{CurvePoint{4200, 120}, CurvePoint{3700, 10}};
static_assert(validate_curve(over_100).error() == CurveError::invalid_percentage);

constexpr std::array duplicate_voltage{CurvePoint{4200, 100}, CurvePoint{4200, 0}};
...
static_assert(estimate_charge_percent(3950, cells).value() == 55);
static_assert(estimate_charge_percent(3900, ascending).error() ==
	      CurveError::invalid_voltage_order);

int main()
{
	CHECK_EQ(curve.percent_at(3950), 55);
	CHECK_EQ(curve.percent_at(5000), 100);
	CHECK_EQ(curve.percent_at(0), 0);

	/* Interpolation is monotonically non-increasing as voltage falls. */
	int previous = 101;
	for (std::int32_t mv = 4300; mv >= 3200; mv -= 10) {
		const int percent = curve.percent_at(mv);
	...
	CHECK_EQ(built->percent_at(3700), 10);
